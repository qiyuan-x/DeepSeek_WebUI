...
async function startServer() {
  ...
  app.get("/api/conversations/:id/messages", (req, res) => {
    ...
    const filtered = msgs.filter((m) => m.conversation_id === req.params.id);
  ...
  app.patch("/api/conversations/:id", (req, res) => {
    ...
    if (idx !== -1) {
      if (req.body.title !== void 0) convs[idx].title = req.body.title;
      if (req.body.system_prompt !== void 0) convs[idx].system_prompt = req.body.system_prompt;
      convs[idx].updated_at = (/* @__PURE__ */ new Date()).toISOString();
      writeJson(CONV_FILE, convs);
    }
    res.json({ success: true });
  });
  app.delete("/api/conversations/:id", (req, res) => {
    try {
      ...
      msgs = msgs.filter((m) => m.conversation_id !== req.params.id);
